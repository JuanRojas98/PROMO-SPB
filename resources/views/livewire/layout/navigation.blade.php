...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
                ...
                @if (Auth::user()->role_id == 1)
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="route('admin.users')" :active="request()->routeIs('admin.users')"
                                    class="text-xl" wire:navigate>
                            {{ __('Usuarios') }}
                        </x-nav-link>
                    </div>
                @elseif (Auth::user()->role_id == 2)
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="route('backoffice.invoices')" :active="request()->routeIs('backoffice.invoices')"
                                    class="text-xl" wire:navigate>
                            {{ __('Facturas') }}
                        </x-nav-link>
                    </div>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            ...
                            <div x-data="{{ json_encode(['first_name' => auth()->user()->first_name]) }}"
                                 x-text="first_name"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @elseif (Auth::user()->role_id == 2)
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-responsive-nav-link :href="route('backoffice.invoices')" :active="request()->routeIs('backoffice.invoices')"
                                class="text-xl" wire:navigate>
                        {{ __('Facturas') }}
                    </x-responsive-nav-link>
                </div>
